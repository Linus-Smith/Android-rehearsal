...
/**
 * Created by Linus on 2017/4/20.
 */

public class CrackingLayoutManager extends RecyclerView.LayoutManager implements
        ItemTouchHelper.ViewDropHandler {
    private static final String TAG = "CrackingLayoutManager";
    ...
    /**
     * @create by linus.yang
     * The location of the Item among the container
     */
    int mCentreX;

    /**
     * @create by linus.yang
     *  The item standard width of the
     */
    //TODO add set The value
    int mItemNormWidth = 90;
     ...
     * the same object.
     */
    private CrackingLayoutManager.LayoutState mLayoutState;

    /**
     * Many calculations are made depending on orientation. To keep it clean, this interface
     * helps {@link CrackingLayoutManager} make those decisions.
    ...
    CrackingLayoutManager.SavedState mPendingSavedState = null;
     ...
     * */
    final CrackingLayoutManager.AnchorInfo mAnchorInfo = new CrackingLayoutManager.AnchorInfo();
     ...
     */
    public CrackingLayoutManager(Context context) {
     ...
     */
    public CrackingLayoutManager(Context context, int orientation, boolean reverseLayout) {
     ...
     */
    public CrackingLayoutManager(Context context, AttributeSet attrs, int defStyleAttr,
                               int defStyleRes) {
    ...
    public Parcelable onSaveInstanceState() {
        if (mPendingSavedState != null) {
            return new CrackingLayoutManager.SavedState(mPendingSavedState);
        }
        CrackingLayoutManager.SavedState state = new CrackingLayoutManager.SavedState();
        if (getChildCount() > 0) {
            ensureLayoutState();
    ...
    public void onRestoreInstanceState(Parcelable state) {
        if (state instanceof CrackingLayoutManager.SavedState) {
            mPendingSavedState = (CrackingLayoutManager.SavedState) state;
            requestLayout();
            if (DEBUG) {
    ...
    /**
     * Sets the orientation of the layout. {@link CrackingLayoutManager}
     ...
     * By default, {@link CrackingLayoutManager} lays out 1 extra page of
        ...
        LinearSmoothScroller linearSmoothScroller =
                new LinearSmoothScroller(recyclerView.getContext()) {
                    ...
                    public PointF computeScrollVectorForPosition(int targetPosition) {
                        return CrackingLayoutManager.this
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (mAnchorInfo.mLayoutFromEnd) {
            firstLayoutDirection = mShouldReverseLayout ? CrackingLayoutManager.LayoutState.ITEM_DIRECTION_TAIL :
                    CrackingLayoutManager.LayoutState.ITEM_DIRECTION_HEAD;
        } else {
            firstLayoutDirection = mShouldReverseLayout ? CrackingLayoutManager.LayoutState.ITEM_DIRECTION_HEAD :
                    CrackingLayoutManager.LayoutState.ITEM_DIRECTION_TAIL;
        }
    ...
    void onAnchorReady(RecyclerView.Recycler recycler, RecyclerView.State state,
                       CrackingLayoutManager.AnchorInfo anchorInfo, int firstLayoutItemDirection) {
    }
...
//            }
            ...
            final int direction = position < firstChildPos != mShouldReverseLayout
                    ? CrackingLayoutManager.LayoutState.LAYOUT_START : CrackingLayoutManager.LayoutState.LAYOUT_END;
            if (direction == CrackingLayoutManager.LayoutState.LAYOUT_START) {
    ...
    private void updateAnchorInfoForLayout(RecyclerView.Recycler recycler, RecyclerView.State state,
                                           CrackingLayoutManager.AnchorInfo anchorInfo) {
    ...
    private boolean updateAnchorFromChildren(RecyclerView.Recycler recycler,
                                             RecyclerView.State state, CrackingLayoutManager.AnchorInfo anchorInfo) {
        if (getChildCount() == 0) {
            return false;
        }
     ...
     */
    private boolean updateAnchorFromPendingData(RecyclerView.State state, CrackingLayoutManager.AnchorInfo anchorInfo) {
        if (state.isPreLayout() || mPendingScrollPosition == NO_POSITION) {
            return false;
        }
    ...
    private void updateLayoutStateToFillEnd(CrackingLayoutManager.AnchorInfo anchorInfo) {
        updateLayoutStateToFillEnd(anchorInfo.mPosition, anchorInfo.mCoordinate);
    }

    private void updateLayoutStateToFillEnd(int itemPosition, int offset) {
        mLayoutState.mAvailable = mOrientationHelper.getEndAfterPadding() - offset;
        mLayoutState.mItemDirection = mShouldReverseLayout ? CrackingLayoutManager.LayoutState.ITEM_DIRECTION_HEAD :
                CrackingLayoutManager.LayoutState.ITEM_DIRECTION_TAIL;
        mLayoutState.mCurrentPosition = itemPosition;
        mLayoutState.mLayoutDirection = CrackingLayoutManager.LayoutState.LAYOUT_END;
        mLayoutState.mOffset = offset;
        mLayoutState.mScrollingOffset = CrackingLayoutManager.LayoutState.SCOLLING_OFFSET_NaN;
    }

    private void updateLayoutStateToFillStart(CrackingLayoutManager.AnchorInfo anchorInfo) {
    ...
    private void updateLayoutStateToFillStart(int itemPosition, int offset) {
        ...
        mLayoutState.mCurrentPosition = itemPosition;
        mLayoutState.mItemDirection = mShouldReverseLayout ? CrackingLayoutManager.LayoutState.ITEM_DIRECTION_TAIL :
                CrackingLayoutManager.LayoutState.ITEM_DIRECTION_HEAD;
        mLayoutState.mLayoutDirection = CrackingLayoutManager.LayoutState.LAYOUT_START;
        mLayoutState.mOffset = offset;
        mLayoutState.mScrollingOffset = CrackingLayoutManager.LayoutState.SCOLLING_OFFSET_NaN;
     ...
     */
    CrackingLayoutManager.LayoutState createLayoutState() {
        return new CrackingLayoutManager.LayoutState();
    }
    ...
    private void updateLayoutState(int layoutDirection, int requiredSpace,
        ...
        int scrollingOffset;
        if (layoutDirection == CrackingLayoutManager.LayoutState.LAYOUT_END) {
    ...
    int scrollBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        ensureLayoutState();
        final int layoutDirection = dy > 0 ? CrackingLayoutManager.LayoutState.LAYOUT_END : CrackingLayoutManager.LayoutState.LAYOUT_START;
     ...
     * @see CrackingLayoutManager.LayoutState#mLayoutDirection
     */
    private void recycleByLayoutState(RecyclerView.Recycler recycler, CrackingLayoutManager.LayoutState layoutState) {
        if (!layoutState.mRecycle || layoutState.mInfinite) {
            return;
        }
        if (layoutState.mLayoutDirection == CrackingLayoutManager.LayoutState.LAYOUT_START) {
     ...
     * independent from the rest of the {@link CrackingLayoutManager}
     ...
     */
    int fill(RecyclerView.Recycler recycler, CrackingLayoutManager.LayoutState layoutState,
             RecyclerView.State state, boolean stopOnFocusable) {
        // max offset we should set is mFastScroll + available
        final int start = layoutState.mAvailable;
        if (layoutState.mScrollingOffset != CrackingLayoutManager.LayoutState.SCOLLING_OFFSET_NaN) {
        ...
        CrackingLayoutManager.LayoutChunkResult layoutChunkResult = new CrackingLayoutManager.LayoutChunkResult();
    ...
    void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state,
                     CrackingLayoutManager.LayoutState layoutState, CrackingLayoutManager.LayoutChunkResult result) {
        ...
        if (layoutState.mScrapList == null) {
            if (mShouldReverseLayout == (layoutState.mLayoutDirection
                    == CrackingLayoutManager.LayoutState.LAYOUT_START)) {
                addView(view);
            } else {
                addView(view, 0);
            }
...
//    }
     ...
     * @return {@link CrackingLayoutManager.LayoutState#LAYOUT_START} or {@link CrackingLayoutManager.LayoutState#LAYOUT_END} if focus direction
     * is applicable to current state, {@link CrackingLayoutManager.LayoutState#INVALID_LAYOUT} otherwise.
     */
    int convertFocusDirectionToLayoutDirection(int focusDirection) {
        switch (focusDirection) {
            case View.FOCUS_BACKWARD:
                if (mOrientation == VERTICAL) {
                    return CrackingLayoutManager.LayoutState.LAYOUT_START;
                } else if (isLayoutRTL()) {
                    return CrackingLayoutManager.LayoutState.LAYOUT_END;
                } else {
                    return CrackingLayoutManager.LayoutState.LAYOUT_START;
                }
            ...
            case View.FOCUS_UP:
                return mOrientation == VERTICAL ? CrackingLayoutManager.LayoutState.LAYOUT_START
                        : CrackingLayoutManager.LayoutState.INVALID_LAYOUT;
            case View.FOCUS_DOWN:
                return mOrientation == VERTICAL ? CrackingLayoutManager.LayoutState.LAYOUT_END
                        : CrackingLayoutManager.LayoutState.INVALID_LAYOUT;
            case View.FOCUS_LEFT:
                return mOrientation == HORIZONTAL ? CrackingLayoutManager.LayoutState.LAYOUT_START
                        : CrackingLayoutManager.LayoutState.INVALID_LAYOUT;
            case View.FOCUS_RIGHT:
                return mOrientation == HORIZONTAL ? CrackingLayoutManager.LayoutState.LAYOUT_END
                        : CrackingLayoutManager.LayoutState.INVALID_LAYOUT;
            default:
                if (DEBUG) {
                    Log.d(TAG, "Unknown focus request:" + focusDirection);
                }
                return CrackingLayoutManager.LayoutState.INVALID_LAYOUT;
        }
    ...
    public View onFocusSearchFailed(View focused, int focusDirection,
        ...
        if (layoutDir == CrackingLayoutManager.LayoutState.INVALID_LAYOUT) {
            return null;
        }
        ensureLayoutState();
        final View referenceChild;
        if (layoutDir == CrackingLayoutManager.LayoutState.LAYOUT_START) {
    ...
    public void prepareForDrop(View view, View target, int x, int y) {
        ...
        final int dropDirection = myPos < targetPos ? CrackingLayoutManager.LayoutState.ITEM_DIRECTION_TAIL :
                CrackingLayoutManager.LayoutState.ITEM_DIRECTION_HEAD;
        if (mShouldReverseLayout) {
            if (dropDirection == CrackingLayoutManager.LayoutState.ITEM_DIRECTION_TAIL) {
            ...
            if (dropDirection == CrackingLayoutManager.LayoutState.ITEM_DIRECTION_HEAD) {
    ...
    public static class SavedState implements Parcelable {
        ...
        public SavedState(CrackingLayoutManager.SavedState other) {
        ...
        public static final Parcelable.Creator<CrackingLayoutManager.SavedState> CREATOR
                = new Parcelable.Creator<CrackingLayoutManager.SavedState>() {
            @Override
            public CrackingLayoutManager.SavedState createFromParcel(Parcel in) {
                return new CrackingLayoutManager.SavedState(in);
            }

            @Override
            public CrackingLayoutManager.SavedState[] newArray(int size) {
                return new CrackingLayoutManager.SavedState[size];
            }
        };
    }
